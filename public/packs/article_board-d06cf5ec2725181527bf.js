...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "d06cf5ec2725181527bf";
...
onPageLoad(['articles#new', 'articles#edit'], function () {
  ...
  uploadArea.addEventListener('drop', function (e) {
    ...
    e.preventDefault();
    e.target.style.backgroundColor = 'pink'; // dt.items.add(e.dataTransfer.files[0]);

    dt.files.add(e.dataTransfer.files[0]);
    fileField.files = dt.files;
    console.log(dt.files); //fileField.files = e.dataTransfer.files;

    setPreviewImage(fileField.files);

    if (dt.files.length >= 3) {
      uploadArea.firstElementChild.textContent = 'これ以上画像を送付できません。';
    }
  });
  clearImagesButton.addEventListener('click', function () {
    ...
    console.log(dt.items);
    setPreviewImage(fileField.files);
    uploadArea.firstElementChild.textContent = '送付したい画像を3枚までドラッグ&ドロップ';
  });
});
...
/******/ });
//# sourceMappingURL=article_board-d06cf5ec2725181527bf.js.map